 ...
 */
	...
	print $vs_control_box = caFormControlBox(
		...
		(caNavButton($this->request, __CA_NAV_BUTTON_CANCEL__, _t("Cancel"), 'client/library', 'OrderEditor', 'CustomerInfo', array('order_id' => $vn_order_id))),
		'',
...
?>
	<h1><?php print _t('User account to associate loan with'); ?></h1>
	<div class="formLabel">
	...
	<script type="text/javascript">
		...
		jQuery('#client_autocomplete').result(function(event, data, formatted) {
			...
			if(data[3]) {
				...
				jQuery.getJSON('<?php print caNavUrl($this->request, 'client/library', 'OrderEditor', 'GetUserProfileInfo'); ?>', { user_id: user_id }, function(d, t, x) { 
