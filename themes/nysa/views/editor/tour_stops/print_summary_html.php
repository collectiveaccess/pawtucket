...
<?php  
						...
						if($this->request->config->get('summary_show_identifier')) {
							print "<span class='headerText'>"._t("Item identifier").": ".$t_stop->get("idno")."</span>"; 
						}
?>
					</td>
				</tr>
...
?>		
		<div style="width:100%; margin-top:10px; text-align: center;">
			<h1><?php print $t_stop->getLabelForDisplay();?></h1>
		</div>
			<?php
				foreach($va_placements as $vn_placement_id => $va_info) {
					if (!is_array($va_bundle_info)) break;
					if (!strlen($vs_display_value = $t_display->getDisplayValue($t_stop, $vn_placement_id, array('purify' => true)))) {
