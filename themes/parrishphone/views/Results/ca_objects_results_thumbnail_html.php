...
if($vo_result) {
	...
	$vs_search = $o_result_context->getSearchExpression();
		...
		if ($this->request->getController() == "Search"){	
			...
			print caNavLink($this->request, 'View as Map', 'modebutton', 'Search', 'Index', 'view/map', array('search' => $vs_search));
		} else {
			print caNavLink($this->request, 'View as List', 'modebutton', 'Browse', 'Index', 'view/thumbnail');
			print caNavLink($this->request, 'View as Map', 'modebutton', 'Browse', 'Index', 'view/map');
		}
		print"</div>";
		...
		while(($vn_item_count < $vn_items_per_page) && ($vo_result->nextHit())) {
			...
			print "<div class='item'><div class='thumb'>".caNavLink($this->request, $vo_result->getMediaTag('ca_object_representations.media', 'icon'), '', 'Detail', 'Object', 'Show', array('object_id' => $vn_object_id))."</div><!-- end thumb -->";
			...
			$this->setVar('caption_date', $vo_result->get("ca_objects.creation_date"));
			$this->setVar('caption_place', $vo_result->get("ca_places.preferred_labels", array('restrictToRelationshipTypes' => 'depicts', 'delimiter' => ', ')));
