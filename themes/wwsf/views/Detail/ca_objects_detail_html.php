 ...
 */
	...
	$vb_is_in_result_list = 	$this->getVar("is_in_result_list");
	global $g_ui_locale;
	...
	if($pn_set_id && $pn_set_id != $this->request->session->getVar("last_set_id")){
		...
		$vb_is_in_result_list = ($o_resultContext->getIndexInResultList($vn_object_id) != '?');
	}
...
<?php
			...
			$va_broadcasters = $t_object->get('ca_entities', array("returnAsArray" => 1, 'checkAccess' => $va_access_values, 'restrict_to_relationship_types' => array('broadcaster')));
			if(sizeof($va_broadcasters) > 0){
				if($g_ui_locale == 'en_US') {
					print "Broadcaster: ";
				} else {
					print "Sender: ";
				}
				$va_broadcasters_display = array();
				foreach($va_broadcasters as $va_broadcaster){
					$va_broadcasters_display[] = caNavLink($this->request, $va_broadcaster["label"], '', '', 'Search', 'Index', array('search' => $va_broadcaster["label"]));
				}
				print implode(", ", $va_broadcasters_display);
				print "<br/>";
			}
			...
			$vn_license_text = $t_object->get("ca_objects.license_type");
 			...
 			// 729 no, 1758 by-sa, 728 by-nc-nd 
 			switch($vn_license_text) {
 				case 729:
 					print '<br/>'._t("License").": ";
				...
				case 728:
 				...
 				case 1758:
					if ($g_ui_locale == 'en_US') {
 					print '<br/>'._t("License").': <a rel="license" href="http://creativecommons.org/licenses/by-sa/3.0/de/deed.en_US" ntarget="_blank"><img alt="Creative Commons License" style="border-width:0" src="http://i.creativecommons.org/l/by-sa/3.0/de/80x15.png" /></a>';
	 				} else {
 						print '<br/>'._t("License").': <a rel="license" href="http://creativecommons.org/licenses/by-sa/3.0/de/" target="_blank"><img alt="Creative Commons License" style="border-width:0" src="http://i.creativecommons.org/l/by-sa/3.0/de/80x15.png" /></a>';
 					}
 					print "<br/>";
	 				$vn_show_download_link = 1;
 					break; 					
			...
			if($this->request->config->get('ca_objects_map_attribute') && $t_object->get($this->request->config->get('ca_objects_map_attribute'))){
				...
				$o_map->mapFrom($t_object, $this->request->config->get('ca_objects_map_attribute'), array('request' => $this->request, 'checkAccess' => $va_access_values));
...
<?php
		if ($vb_is_in_result_list) {
			...
			$vs_back_link = ResultContext::getResultsLinkForLastFind($this->request, 'ca_objects', "<img src='".$this->request->getThemeUrlPath()."/graphics/arrow_up_lg.gif' width='13' height='12' border='0' id='buttonBack'>", '');
			print "&nbsp;&nbsp;&nbsp;".($vs_back_link ? $vs_back_link : '')."&nbsp;&nbsp;&nbsp;";
			if ($vn_next_id) {
...
<?php
					...
					if($vn_show_download_link){
						print caNavLink($this->request, _t("DOWNLOAD +"), '', 'Detail', 'Object', 'DownloadRepresentation', array('representation_id' => $t_rep->getPrimaryKey(), "object_id" => $vn_object_id, "version" => 'original'), array('style' => 'margin-right:30px;'));
					}
					if($pn_set_id){
					...
					if ($vn_show_zoom) {
						print "<a href='#' onclick='caMediaPanel.showPanel(\"".caNavUrl($this->request, 'Detail', 'Object', 'GetRepresentationInfo', array('object_id' => $t_object->get("object_id"), 'representation_id' => $t_rep->getPrimaryKey()))."\"); return false;' >"._t("Zoom")." +</a>";
					}
?>
				</div>			
