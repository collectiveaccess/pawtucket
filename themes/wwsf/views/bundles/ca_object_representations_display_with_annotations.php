...
<?php							
				$va_annotations = $this->getVar('annotations');
					foreach($va_annotations as $va_annotation) {
						print "<li class='videoViewerClipList'><div class='videoViewerClipListTimecode'>[".$va_annotation['startTimecode']."]</div>";
						print "<a href='#' onclick='flowplayer(0).seek(".$va_annotation['startTimecode_raw']."); return false;'>".$va_annotation['label']."</a></li>";
					}
?>
			</ul>
		</div>
...
<script type="text/javascript">
        function openClipList() {
        		if (jQuery('#clipListContainer').css('left') == '598px') {
                        // opened... so close
