 ...
 */
	...
	require_once(__CA_APP_DIR__."/controllers/SplashController.php");
	...
	$t_featured->load(array('set_code' => $this->request->config->get('browse_set_name')));
	...
	if((sizeof($va_access_values) == 0) || (sizeof($va_access_values) && in_array($t_featured->get("access"), $va_access_values))){
		...
		$va_featured_ids = array_keys(is_array($va_tmp = $t_featured->getItemRowIDs(array('checkAccess' => array(1)))) ? $va_tmp : array());	// These are the object ids in the set
	}
...
if (sizeof($va_criteria)) {
				...
				if (sizeof($va_facets)) { 
?>
					<div id="refineBrowse"><span class='refineHeading'><?php print _t('or refine results by'); ?>:</span>
<?php
						$vn_i = 1;
...
<?php
				...
				print "</div><!-- end browseRight -->";
				print "<div id='browseLeft'>";
				if(is_array($va_random_items) && sizeof($va_random_items)){
...
?>
		...
		<div id="browseFeatured">
		<div id="browseFeaturedTitle">Featured Collections</div>
<?php

		foreach ($va_featured_ids as $featured_id) {
			$t_collection = new ca_collections($featured_id);
			print "<div class='item'>".caNavLink($this->request, $t_collection->get('ca_collections.preferred_labels.name'), '', 'Detail', 'Collection', 'Show', array('collection_id' => $featured_id))."</div>";
