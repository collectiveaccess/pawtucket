<?php
/* ----------------------------------------------------------------------
 * pawtucket2/themes/default/views/ca_tour_stops_search_html.php 
 * ----------------------------------------------------------------------
 * CollectiveAccess
...
<?php
	if($vo_result) {
?>
		<h1><?php print ("Tour stops from the Digital Archive"); ?></h1>
<?php
...
<?php
		...
		if ((!$vs_view) || ($vo_result->numHits() == 0) || (!in_array($vs_view, array_keys($this->getVar('result_views'))))) {
			print $this->render('Results/ca_tour_stops_search_no_results_html.php');
		}else{
			print $this->render('Results/ca_tour_stops_results_'.$vs_view.'_html.php');
		}
?>		
	</div><!-- end sectionbox -->
<?php
