 ...
 */
	...
	if ($this->request->config->get('do_secondary_search_for_ca_special_events')) {
		$qr_occurrences = $this->getVar('secondary_search_ca_special_events');
		if (($vn_num_hits = $qr_occurrences->numHits()) > 0) {
			$vn_num_hits_per_page 	= $this->getVar('secondaryItemsPerPage');
			$vn_page 				= $this->getVar('page_ca_special_events');
			
?>
			<div class="searchSec" id="specialSecondaryResults">
<?php
			
?>
				<h1><?php print _t('Special Events'); ?></h1>
				<div class="searchSecNav">
<?php
					if ($vn_num_hits > $vn_num_hits_per_page) {
						print "<div class='nav'>";
						if ($vn_page > 0) {
							print "<a href='#' onclick='jQuery(\"#specialSecondaryResults\").load(\"".caNavUrl($this->request, '', 'Search', 'secondarySearch', array('spage' => $vn_page - 1, 'type' => 'ca_special_events'))."\"); return false;'>&lsaquo; "._t("Previous")."</a>";
						}
						if (($vn_page > 0) && ($vn_page < (ceil($vn_num_hits/$vn_num_hits_per_page) - 1))) {
							print " | ";
						}
						if ($vn_page < (ceil($vn_num_hits/$vn_num_hits_per_page) - 1)) {
							print "<a href='#' onclick='jQuery(\"#specialSecondaryResults\").load(\"".caNavUrl($this->request, '', 'Search', 'secondarySearch', array('spage' => $vn_page + 1, 'type' => 'ca_special_events'))."\"); return false;'>"._t("Next")." &rsaquo;</a>";
						}
						print "</div><!-- end nav -->\n";
					}
